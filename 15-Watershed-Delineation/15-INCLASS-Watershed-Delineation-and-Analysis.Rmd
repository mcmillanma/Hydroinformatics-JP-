...
library(raster)
library(sf)
library(whitebox) #spatial analyses (not R package)
library(tmap)
library(stars)
library(rayshader)
library(rgl)
...
filled_breached <-
  raster("McDonaldHollowDEM/bmstationdem_filled_breached.tif")
```
...
wbt_d8_flow_accumulation(
  input = "McDonaldHollowDEM/bmstationdem_filled_breached.tif",
...
d8pointer <- raster("McDonaldHollowDEM/D8pointer.tif")
```
...
# whitebox takes files not R object
```
...
ws4shp <- wsshape %>% filter(brush_watersheds == "4")
...
tm_shape(hillshade) +
  ...
  tm_layout(title = "Mine Watershed 33.53 ha", title.color = "red", title.position = c("center", "top"))

x <- shapefile("McDonaldHollowDEM/pourpoints.shp") 
crs(x)
...
```{r}
wsElev <- raster::extract(dem, wsshape)

wsElevs1 <- setNames(wsElev, c("WS1", "WS2", "WS3", "WS4"))

WS1 <- as_tibble(wsElevs1$WS1)
WS2 <- as_tibble(wsElevs1$WS2)
WS3 <- as_tibble(wsElevs1$WS3)
WS4 <- as_tibble(wsElevs1$WS4)
...
#convert to matrix
wsmat <- matrix(
  raster::extract(wsmask, extent(wsmask)),
  nrow = ncol(wsmask),
  ncol = nrow(wsmask))
...
Create a map showing the watershed for the point shown in the image above. The green box highlights the watershed to further clarify, it should be the smaller watersshed nestled between the the lower portion of the two included in the green box. Use the DEM from the class exercise.

Turn in the following:
An OUTLINE of the watershed (not shaded like in the image above) (6 points)

A scale bar and north arrow (1 point)

A hillshade under the watershed outline (1 point)

A title that says "Mine Watershed xx ha" where xx is replaced with the area of the watershed, calculated from the watershed you created. (2 points)

```{r}
# st_area() from stars package to get area of watershed
