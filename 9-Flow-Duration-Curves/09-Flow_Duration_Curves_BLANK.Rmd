...
Qdat %>% ggplot(aes(Flow)) +
  stat_ecdf() +
  scale_x_log10() +
  geom_vline(xintercept = median(Qdat$Flow),
             color = "red") +
  geom_vline(xintercept = quantile (Qdat$Flow) [4],
             color = "blue")
```
...
```{r}
#flowecdf <-ecdf(Qdat$Flow) this outputs a function and inside function you can put a value and it will tell you the exceedance value i.e. flowecdf(1000) = [1] 0.999087
...
#basically same thing as above but x values
...
#case_when dplyr function when specific condition is true you get set value
```
...
# forcing to be certain siz fig ^

Qdat %>% ggplot(aes(x = period, y = Flow)) +
  geom_boxplot() +
  scale_y_log10()

# boxplots not good for looking at differences in high and low flow; reason why we use flow duration curves
```
...
```{r}
# stack on top using patchwork to make one composite figure; paste plot and answer into canvas; need to figure out gauge number

#find site no
AZsites <- whatNWISdata(service = "dv",
                           stateCd = "AZ",
                           parameterCd = "00060")
...
QCR <-readNWISdv(siteno, parameter, startDate, endDate) %>%
  renameNWISColumns()

QCR <- QCR %>%
  mutate( year = year(Date)) %>%
  mutate(period = case_when(year >= 1905 & year <=1936~"1905-1936",
                            year >= 1937 & year <= 1965 ~"1937-1965"))  %>%
  group_by(period) %>%
  mutate(rank = rank(-Flow)) %>%
  mutate(P = 100*(rank/(length(Flow) +1)))

FDC <- QCR %>% ggplot(aes(x = P, y= Flow, color = period)) +
  geom_line() +
  xlab("% Time Flow is equaled or exceeded")+
  ylab("Q (cfs)")
FDC
...
Flow <- QCR %>% 
  ggplot(aes(Date, Flow, color = period)) +
  geom_line() +
  ylab("Q (cfs)")
Flow
