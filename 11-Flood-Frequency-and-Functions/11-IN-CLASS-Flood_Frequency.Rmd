...
```{r}
peakflows <- peakflows %>%
  mutate(ptheoretical = exp (-exp(-((peak_va-u)/alpha)))) %>%
  mutate(Tptheoretical = (1 / (1 - ptheoretical))) 

peakflows %>% ggplot(aes(x = Tp, y = peak_va, color = "Estimated")) +
  geom_point()+
...
```{r}
peakflows %>% ggplot(aes(x = peak_va, y = Tp, color = "Estimated")) +
  geom_point() +
  geom_line(aes(x = peak_va, y = Tptheoretical, color = "Theoretical"))+
  ylab("Return Period")+
  xlab("Annual Peak Flows")+
...
```{r}
add1 <- function(x){x +1} #{calc <- number+1  calc} means return calc
add1(x = 7)
add1(7)
...
hundredyear <- function(gageid, startdate, enddate){
  gageq <- readNWISpeak(siteNumbers = gageid, startDate = startdate, endDate = enddate)
  xbar = mean(peakflows$peak_va)
  sx = sd(peakflows$peak_va)
  alpha = (sqrt(6)*sx) / pi
  u = xbar - (0.5772 * alpha)
...
hundredyear("03171000", "1945-01-01", "2022-03-17")
