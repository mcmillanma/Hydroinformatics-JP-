...
mydata <- tribble(
 ...
 pivot_longer(mydata, c(Temp, Discharge))
...
Qdat %>%
  ...
  geom_line() +
  geom_line (aes(y= xdaymean, color = "rolling mean"))
```
...
QyearMins <- Qdat %>% mutate(year = year(Date)) %>%
  group_by(year) %>%
  summarize(minQ = min(xdaymean, na.rm = T), 
            lenDat = length(Flow) , 
            lenNAs = sum(is.na(xdaymean))) %>%
  filter(lenDat >328 &lenNAs/lenDat < 0.1)
# drop anything less than 328 days or if the year has more than 10% NAs```
...
ggplot(QyearMins, aes(ReturnInterval, minQ)) +
  geom_point()
```
...
```{r}
 QyearMins %>%
  ggplot(aes(x = ExceedProb, y= minQ)) +
  geom_point()
```
...
Xbar <- mean(log10(QyearMins$minQ))
S <- sd(log10(QyearMins$minQ))
g <- skewness(log10(QyearMins$minQ))
...
```{r}
QyearMins %>%
  ggplot(aes(ReturnInterval, minQ, color = "Estimated")) +
  geom_point()+
  geom_line(aes(y = Qfit, color = "Fitted")) +
  scale_x_log10() +
  ylab("7 day yearly minimum")+
...
Xm1 <- QyearMins$minQ[QyearMins$rank == m1]
Xm2 <- QyearMins$minQ[QyearMins$rank == m2]
