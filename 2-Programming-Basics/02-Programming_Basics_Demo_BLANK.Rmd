# tR Tidyverse Programming Basics {#Programming}
...
```{r}
z <- c(1,3,6)
#unexpected code means something is wrong in syntax
...
```{r}
name <- "Mel"
#characters need "", if not in "" thinks its a variable giving object not found code because it is not in the environment
```
...
#do not use read.csv!!!! It causes errors because it dosen't read as tiblle or do anything with dates. read_csv does unless data is in ambiguous format; R needs to be able to determine date from month and 4 digit year. if data ambiguous, needs addressed in code
...
```{r echo=TRUE}
# == means is this equal to instead of = set it equal to
...
z %in% c(1,3)
# is any variable in z 1 or 3

filter(rbi, STATE == "ME")
# did not create new dataframe, just spits it out
```
...
```{r echo=TRUE}
arrange (rbi, RBI)
#change variable to factor to order; specify factors
```
...
# recommendation to reference by name; select( rbi, 1:4) should work but name prevents future errors incase columns change
#Summary: change plot type, change theme, manipulate and plot data,filter data, assign data to arguments, 
```
...
#this does not change tibble because not set equal to anything
```
...
```{r echo=TRUE}
 summarize(rbi, meanrbi = mean(RBI), maxrbi= max(RBI), minrbi = (RBI))
```
...
#ungroup() if you don't know how it is grouped or str()
summarise(rbistate, meanrbi = mean(RBI), maxrbi= max(RBI), minrbi = (RBI))
```
...
> pbbread(ingredients) is the same as ingredients %\>% pbbread()
>
> ingredients %\>%\
> pbbread() %\>%\
> jbread() %\>%\
...
```{r echo=TRUE}
rbi %>% group_by(STATE) %>%
  summarize(meanrbi = mean (RBI))
# take rbi and do all following operations by STATE, now summarize RBI mean data in new column

# ggplot(mydata, aes(x,y)) same as mydata %>% ggplot(aes(x,y))
```
...
```{r echo=TRUE}
stateRBIs <- rbi %>% group_by(STATE) %>%
  summarize(meanrbi = mean (RBI), maxrbi= max(RBI), minrbi = (RBI))
#same as
rbi %>% group_by(STATE) %>%
  summarize(meanrbi = mean (RBI), maxrbi= max(RBI), minrbi = (RBI))-> stateRBIs
...
mean(x, na.rm = TRUE)
#removes na to give mean
#drop_na() removes the whole row
