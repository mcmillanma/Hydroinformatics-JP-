...
#ModelOutput$Qobs1 <- Qobs #-or-
ModelOutput <- bind_cols(ModelOutput, Qobs1)
#ModelOutput <- ModelOutput %>% mutate(Qobs1 = Qobs) #-or- cbind()
...
```{r}
Outputtrim <- filter(ModelOutput,
                     DATE>=mdy("01-01-2011"))

performance <- Outputtrim %>%
ggplot(aes(DATE, WS_3, color = "Measured")) +
  geom_line() +
  geom_line(aes(y = q, color = "Modelled"))+
  ylab("Discharge (mm/day)")

#ignore first half of model so it can set trends; model starts as watershed dry and needs time to equilibriate
...
#hard code parameters 
params <- c(10,     #FCM ax soil moisture storage, field capacity
            1,      #beta Shape coefficient governing fate of water input to soil moisture storage
            0.3,    #LP Threshold for reduction of evap
            0.5,    #SFCF Snowfall correction factor
            -1,   #TT Threshold temperature
            1.2,      #CFMAX Degree-day factor
            0.35,   #k0 Recession constant (upper storage, near surface)
            0.02,   #k1 Recession constant (upper storage)
            0.01,  #k2 Recession constant (lower storage)
