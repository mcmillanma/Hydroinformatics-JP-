...
for(x in 1:10){val[x] <- x}

days <- c("mon", "tues", "wednes")

for(x in 1:length(days)){ days[x] <- paste0(days[x], "day")}
...
for(x in 1:length(values)) {values[x] <- values[x] + 1}
...
FC    <- runif(N, min = 40, max = 400)  #Max soil moisture storage, field capacity r-unif run a uniform distribution (equal probability of getting any number in the range)
