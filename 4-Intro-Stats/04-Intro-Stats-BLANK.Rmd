...
ExNorm <- rnorm(1000, mean = 5) %>%
  as_tibble() # needs to be tibble for ggplot
...
sample_of_water <- sample(all_the_water$value,
                          size = 100,
                          replace = FALSE) %>%
  as_tibble()
  
samplepdf <- sample_of_water %>%
  ggplot(aes(value)) +
  stat_density() +
  ggtitle("Sample") +
  xlim (1,10)

samplehist <- sample_of_water %>%
  ggplot(aes(value)) +
  geom_histogram() +
  ggtitle("Sample") +
  xlim (1,10)
...
# scale_lim
# $ anything with columns, reference one column (dataframe$column)
samplepdf + population_hist + samplehist
...
#list.files shows files in console so you can copy and paste
...
weights <- c(0.20, 0.15, 0.40, 0.15, 0.10)
mean(precip)
sum (precip * weights)
...
pineQ <-read.csv("Pine_Jan-Mar_2010.csv")
...
```{r}
shapiro.test(pineQ$cfs) #not normal

shapiro.test(rnorm(100)) #normal
