...
#"" highlight one quote makes character to keep leading 0
site <- "03171000"
...
dataAvailable <- whatNWISdata(siteNumber = site, 
                              service = "dv", 
                              statCd = "00003")
...
dataAvailable <- dataAvailable %>%
  select(site_no, station_nm, parm_cd, srsname, parameter_units, begin_date, end_date)
```
...
swva_sites <- whatNWISsites(bBox = swva,
                            parameterCd = c("00060", "00010"), hasDataTypeCd = "dv")
...
smallVA <-readNWISdata (service = "dv", stateCD = "VA", parameterCd ="00060", drainAreaMax = "20",
                        statCd = "00003")
...
swva_dat <- readNWISdv (siteNumber = swva_sites$site_no, parameterCd = params, startDate = start, endDate = end) %>% renameNWISColumns()
```
...
swva_dat %>%
  ggplot(aes(Date, Wtemp, color = site_no)) +
  geom_line() +
  theme_classic()
...
```{r}
# join these and color same graph by site name instead of number swva_dat, swva_sites 

swva_dat_clean <- left_join(swva_dat, swva_sites, by = "site_no") 
  swva_dat_clean %>%
    ggplot(aes(Date, Wtemp, color = station_nm)) +
  geom_line() +
  xlab(element_blank()) +
  ylab("Water temperature (deg C)")+
  labs(color = "Gauge Sites")

# xlab(element_blank()) removes label
...
```{r}
#unique() gives values in column
#data$ in console brings up 
swva_wide <- swva_dat_clean %>%
  select(station_nm, Flow, Date) %>%
  pivot_wider(names_from = station_nm, values_from = Flow)

swva_wide <- swva_wide %>%
  mutate(Radford_Ivanhoe = `NEW RIVER AT RADFORD, VA`- `NEW RIVER AT IVANHOE, VA`)

swva_wide %>%
  ggplot(aes(Date,Radford_Ivanhoe )) +
  geom_line() +
...
swva_chem_site <- whatNWISsites(bBox = swva, parameterCD = params)

#filter to just stream water
swva_chem_site <- filter(swva_chem_site, site_tp_cd == "ST")

wqdat <- readNWISqw (siteNumber = swva_chem_site$site_no, parameterCd = params)
...
```{r}
wqdat_clean <- wqdat %>%
  left_join(parameterCdFile, by = c("parm_cd" = "parameter_cd")) %>% left_join(swva_chem_site, by = "site_no") %>% select(station_nm, sample_dt, sample_tm, result_va, srsname, parameter_units) %>%
  filter(str_detect(station_nm, "NEW RIVER"))


# copied from completed not done in class
